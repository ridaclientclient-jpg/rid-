...
function MarketplaceResetPasswordForm() {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setTimeout(() => router.push('/marketplace/login'), 2000);
    } catch {
      ...
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            ...
            <button onClick={() => router.push('/marketplace/recovery')} className="w-full bg-amber-500 text-white font-medium py-3 rounded-xl hover:bg-amber-600 transition-colors">
      ...
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            ...
            <button onClick={() => router.push('/marketplace/login')} className="w-full bg-amber-500 text-white font-medium py-3 rounded-xl hover:bg-amber-600 transition-colors">
              Iniciar Sesion
            </button>
          </div>
        </motion.div>
    ...
    <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
      <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Cuenta de tienda</p>
        </div>
        ...
        <div className="glass-strong rounded-2xl p-6 space-y-4">
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Minimo 6 caracteres" className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-12 py-3 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-amber-500 transition-colors" />
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
              <input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repite la contrasena" className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-amber-500 transition-colors" />
            </div>
          </div>
          ...
          <button onClick={handleReset} disabled={isLoading || !newPassword || !confirmPassword} className="w-full bg-amber-500 text-white font-medium py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50 hover:bg-amber-600 transition-colors">
            {isLoading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Actualizando...</>) : 'Cambiar Contrasena'}
          </button>

          <button onClick={() => router.push('/marketplace/login')} className="w-full flex items-center justify-center gap-2 text-gray-400 hover:text-white py-2">
