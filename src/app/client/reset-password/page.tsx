...
import { Lock, Eye, EyeOff, CheckCircle, Zap, ArrowLeft, Loader2 } from 'lucide-react';
    ...
    const checkSession = async () => {
      try {
        ...
        if (session || hashSession) {
          ...
          // Check if user is already logged in (came from profile change password)
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error?.includes('session') || data.error?.includes('expired') || data.error?.includes('JWT')) {
          ...
          setTimeout(() => router.push('/client/recovery'), 2000);
          return;
        }
      ...
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            ...
            <button
              onClick={() => router.push('/client/recovery')}
              className="w-full btn-neon text-white font-medium py-3 rounded-xl"
            >
    ...
    return (
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            <p className="text-sm text-gray-400">Tu contrasena se ha cambiado exitosamente. Ya puedes iniciar sesion con tu nueva contrasena.</p>
            <button
    ...
    <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
      <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Ingresa tu nueva contrasena</p>
        </div>
