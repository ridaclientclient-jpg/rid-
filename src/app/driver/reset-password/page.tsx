...
import { Lock, Eye, EyeOff, CheckCircle, Car, ArrowLeft, Loader2 } from 'lucide-react';
      ...
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            ...
            <button onClick={() => router.push('/driver/recovery')} className="w-full btn-neon text-white font-medium py-3 rounded-xl">
      ...
      <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="glass-strong rounded-2xl p-6 text-center space-y-4">
            ...
            <button onClick={() => router.push('/driver/login')} className="w-full btn-neon text-white font-medium py-3 rounded-xl">
              Iniciar Sesion
            </button>
          </div>
        </motion.div>
    ...
    <div className="min-h-screen bg-rida-dark flex flex-col items-center justify-center p-6">
      <motion.div className="w-full max-w-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Cuenta de conductor</p>
        </div>
        ...
        <div className="glass-strong rounded-2xl p-6 space-y-4">
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-12 py-3 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-cyan-500 transition-colors"
              />
          ...
          <button onClick={handleReset} disabled={isLoading || !newPassword || !confirmPassword} className="w-full btn-neon text-white font-medium py-3 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50">
            {isLoading ? (<><Loader2 className="w-4 h-4 animate-spin" /> Actualizando...</>) : 'Cambiar Contrasena'}
          </button>

          <button onClick={() => router.push('/driver/login')} className="w-full flex items-center justify-center gap-2 text-gray-400 hover:text-white py-2">
