...
const RANDOM_ORIGINS = ['Multiplaza Escazu', 'Hospital Calderon Guardia', 'CClin Central', 'Mall San Pedro', 'Plaça Roosevelt', 'Terminal 7-10'];
...
export default function DriverRides() {
  ...
  const handleAccept = useCallback((ride: PendingRide) => {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-gray-400 mt-1">{isOnline ? 'Estas en linea' : 'Estas fuera de linea'}</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        ...
        >
          ...
          <div className="text-left flex-1">
            ...
            <p className="text-xs text-gray-400">{isOnline ? 'Recibiendo solicitudes de viaje' : 'No recibirás solicitudes'}</p>
          </div>
        ...
        {activeRide && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[activeRide.status]}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-white">{activeRide.riderName}</p>
                <p className="text-xs text-gray-400">{activeRide.riderPhone}</p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => toast.info('Llamando al pasajero...')} className="p-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 transition-colors">
                  <Phone className="w-4 h-4 text-emerald-400" />
                </button>
                <button onClick={() => toast.info('Abriendo chat...')} className="p-2 rounded-xl bg-blue-500/20 hover:bg-blue-500/30 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="glass rounded-xl p-2 text-center">
                ...
                <p className="text-sm font-bold text-white">{activeRide.distance} km</p>
              </div>
              <div className="glass rounded-xl p-2 text-center">
            ...
            <div className="flex gap-3">
              {activeRide.status === 'started' ? (
                ...
                >
                  {activeRide.status === 'assigned' ? 'Iniciar Camino' : 'Iniciar Viaje'}
            ...
            {activeRide.status !== 'started' && (
              <button
                onClick={() => { toast.info('Abriendo navegador...'); }}
      ...
      {!activeRide && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-gray-400">
            {isOnline ? 'Solicitudes Disponibles' : 'Conectate para ver solicitudes'}
          </h2>
          ...
          {pendingRides.length === 0 && isOnline && (
            ...
            >
              ...
              <p className="text-sm text-gray-500">Esperando solicitudes de viaje...</p>
              <p className="text-xs text-gray-600 mt-1">Las solicitudes apareceran automaticamente</p>
            </motion.div>
          )}
          ...
          {pendingRides.length === 0 && !isOnline && (
            ...
            >
              ...
              <p className="text-sm text-gray-500">Conectate para recibir solicitudes</p>
            </motion.div>
          )}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-white">{ride.riderName}</p>
                      <p className="text-xs text-gray-500 flex items-center gap-1">
                        <Star className="w-3 h-3 text-amber-400 fill-amber-400" /> {ride.rating}
                      </p>
                    </div>
                  </div>
                  <p className="text-lg font-bold text-cyan-400">₡{ride.price.toLocaleString()}</p>
                </div>
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> {ride.distance} km</span>
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> ~{ride.distance * 3} min</span>
                </div>
